...
const Movies = ({ moviesData }) => {
  ...
  const loader = (
    <p className="text-2xl text-orange-600 mx-auto">Loading Movies...</p>
  );
  return (
    <>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-10 mx-auto">
          <h1 className="font-medium text-4xl text-gray-900 m-5 text-center">
            Movies List from
            <span className="font-medium text-4xl text-cyan-500 text-center">
              TheMovieDB
            </span>
          </h1>
          <div className="flex flex-wrap -m-4">
            {loading
              ...
              : Object.entries(movies).map((movie) => (
                  <div
                    key={movie[1].id}
                    className="p-4 lg:w-1/4 md:w-1/2 sm:w-full"
                  >
                    <div className="h-full flex flex-col items-center text-center w-full">
                      <img
                        alt="image not found"
                        className="block rounded-lg w-full h-56  object-cover object-center mb-4"
                        src={
